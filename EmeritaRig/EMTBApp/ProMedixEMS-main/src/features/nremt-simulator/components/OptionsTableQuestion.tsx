...
interface OptionsTableQuestionProps {
  question: {
    id: string;
    stem: string;
    instruction?: string;
    ...
    onAnswer: (categorization: Record<string, string>) => void;
  };
}

const OptionsTableQuestion: React.FC<OptionsTableQuestionProps> = ({ question }) => {
  ...
  const checkAnswer = () => {
    const correct = JSON.stringify(categorization) === JSON.stringify(question.correctCategorization);
    setIsCorrect(correct);
    setSubmitted(true);

    // Submit after a brief delay to show feedback
    setTimeout(() => question.onAnswer(categorization), 1500);
  };
  ...
  const showCorrectAnswer = () => {
    setCategorization(question.correctCategorization);
  };

  // Group items by category for display
  const itemsByCategory = question.categories.reduce((acc, category) => {
    acc[category] = question.items.filter(item => categorization[item] === category);
    return acc;
  }, {} as Record<string, string[]>);

  // Items not yet categorized
  const uncategorizedItems = question.items.filter(item => !categorization[item]);
  ...
  return (
    <div className="space-y-6">
      ...
      {uncategorizedItems.length > 0 && (
        <div>
          <h4 className="text-lg font-semibold text-slate-300 mb-3">Items to Categorize:</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {question.categories.map((category) => (
          <div
            ...
            className={`p-4 rounded-lg border-2 border-dashed min-h-[200px] transition-all ${
              draggedItem
      ...
      <div className="bg-slate-800/50 p-3 rounded-lg">
        <div className="flex justify-between items-center text-sm">
          <span className="text-slate-400">
            Categorized: {question.items.filter(item => categorization[item]).length} of {question.items.length}
          </span>
          <span className="text-slate-400">
            {Math.round((question.items.filter(item => categorization[item]).length / question.items.length) * 100)}% complete
          </span>
        </div>
      </div>
        ...
        {!submitted ? (
          <div className="flex justify-between items-center">
            <div className="text-sm text-slate-400">
              Drag items from the list above into the appropriate categories
            </div>
            <button
              onClick={checkAnswer}
              disabled={question.items.some(item => !categorization[item])}
          ...
          <div className="space-y-4">
            <div className={`text-center p-4 rounded-lg ${
              ...
              <div className={`text-2xl font-bold mb-2 ${
                ...
                {isCorrect ? '✓ Correct!' : '✗ Incorrect Categorization'}
              </div>
              <div className="text-slate-300">
                {isCorrect
                  ? 'All items are correctly categorized!'
                  : 'Some items are in the wrong categories.'
                }
              </div>
            </div>
...
export default OptionsTableQuestion;